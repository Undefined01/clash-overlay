...
3. 先执行代理预处理模块（可异步）：例如前置/落地检测、节点重命名
4. 再按注册顺序执行配置模块，每个模块返回其贡献的配置片段
5. `moduleMerge` 合并所有片段——数组按排序值拼接，对象深合并，标量做冲突检测
6. 解析所有 `deferred` 值（如依赖最终代理列表的代理组成员）
7. 清理元数据（`_proxies`、`_allSelectables` 等），输出最终配置
...
```bash
# 构建
pnpm build    # → packages/substore-overlay/dist/override.js
...
| 模块 | 文件 | 职责 |
|------|------|------|
| entry-landing-geo | `modules/entry-landing-geo.ts` | 检测前置/落地地区，写入 `_geoEntry/_geoLanding` |
| rename-by-entry-landing | `modules/rename-by-entry-landing.ts` | 按 `_geo*` 字段统一重命名节点 |
...
| `entry_landing_geo_enabled` | `boolean` | `true` | 是否启用前置/落地检测 |
| `entry_landing_rename_enabled` | `boolean` | `true` | 是否启用基于检测结果重命名 |
| `entry_landing_api` | `string` | `ip-api` 查询 URL | 落地 API（可替换） |
| `entry_landing_surge_http_api` | `string` | `''` | 远端 Surge HTTP API 地址（可选） |
| `entry_landing_surge_http_api_protocol` | `string` | `'http'` | Surge API 协议 |
| `entry_landing_surge_http_api_key` | `string` | `''` | Surge API 密钥 |
| `entry_landing_concurrency` | `number` | `10` | 检测并发数 |
| `entry_landing_timeout` | `number` | `5000` | 检测超时（毫秒） |
| `entry_landing_retries` | `number` | `1` | 检测重试次数 |
| `entry_landing_retry_delay` | `number` | `800` | 重试间隔（毫秒） |
| `entry_landing_default_multiplier` | `number` | `1` | 名称未命中倍率时的默认倍率 |
| `entry_landing_show_residential` | `boolean` | `true` | 是否显示“家宽”标签 |
...
substore-overlay 使用 `createModuleMerge({ uniqueKeyFields: ['rule-providers'] })` 创建合并函数。这意味着：
...
这些策略都由 liboverlay 的 `moduleMerge` 提供，substore-overlay 只是添加了 `rule-providers` 的唯一性约束。详见 [liboverlay 文档](../liboverlay/README.md)。
