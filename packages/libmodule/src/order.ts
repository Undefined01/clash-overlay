...
export function mkBefore<T>(items: T[]): Ordered<T> {
    return { [MARKER]: 'order', order: BEFORE_ORDER, items };
}
...
export function mkAfter<T>(items: T[]): Ordered<T> {
    return { [MARKER]: 'order', order: AFTER_ORDER, items };
}
